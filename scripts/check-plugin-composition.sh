...
# Пробел, найденный этим решением, — прежние проверки этого файла ищут только
# литералы MCP-имён операций встреч в тексте промт-слоя и не заглядывают внутрь
# .mcp.json вообще; без отдельной проверки регресс («кто-то вернул декларацию
# сервера») не поймал бы ни один существующий гейт. Условие: файл либо отсутствует,
# либо не содержит ключ "ktalk" внутри объекта "mcpServers".
check_no_mcp_server() {
    local f=".mcp.json"
    [ -f "$f" ] || return 0
    if grep -Eq '"mcpServers"' "$f" && grep -Eq '"ktalk"[[:space:]]*:' "$f"; then
        echo "FAIL: .mcp.json декларирует MCP-сервер \"ktalk\" — плагин обязан не объявлять MCP-поверхность (ADR-022 Д1)"
        fail=1
    fi
}
check_no_mcp_server
