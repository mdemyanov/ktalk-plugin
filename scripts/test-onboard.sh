...
"$SCRIPT" check >/dev/null 2>&1; check_eq 11 $? "check: 0.4.0 < 0.8.1 → 11"

# 4. версия достаточна → 0
make_env; stub uv 0 ""; stub ktalk 0 "ktalk-mcp 0.8.1"
"$SCRIPT" check >/dev/null 2>&1; check_eq 0 $? "check: 0.8.1 → 0"
...
printf 'allow_install = true\nallow_update = true\n' > "$XDG_CONFIG_HOME/ktalk/onboarding.toml"
stub ktalk 0 "ktalk-mcp 0.4.0"
stub_uv_installs 0.8.1 "Updated ktalk-mcp v0.4.0 -> v0.8.1"
...
printf 'allow_install = true\n' > "$XDG_CONFIG_HOME/ktalk/onboarding.toml"
stub_uv_installs 0.8.1
OUT="$("$SCRIPT" install --json 2>/dev/null)"; check_eq 0 $? "install: индекс отдал 0.8.1 → 0"
printf '%s' "$OUT" | grep -q '"status":"ok"'; check_eq 0 $? "install --json: статус ok при успехе"
printf '%s' "$OUT" | grep -q '"installed_version":"0.8.1"'; check_eq 0 $? "install --json: installed_version при успехе"
...
make_env; mkdir -p "$XDG_CONFIG_HOME/ktalk"
printf 'allow_install = true\n' > "$XDG_CONFIG_HOME/ktalk/onboarding.toml"
stub_uv_installs_off_path 0.8.1
