...
run_secret_scan_tree_gate() {
  ...
  if [[ "$SECRET_SCAN_ENABLED" == "true" ]]; then
    ...
    # напрямую: gitleaks сверяет allowlist-паттерны с путём, КАКИМ он ему передан (§1.6/§2.1
    # ADR-062-spec) — абсолютная цель делает путь целиком (включая ancestor-каталоги выше
    # $REPO_ROOT) кандидатом на совпадение с неякорным паттерном. У дерева `nauta` рабочие
    # копии этой же волны сами лежат под `.worktrees/<имя>` — при абсолютной цели паттерн
    # `(^|/)\.worktrees/` совпадал бы с ПРЕФИКСОМ собственного $REPO_ROOT любого такого
    # дерева и молча схлопывал бы область до 0 байт (поймано Д7 на этом самом дереве при
    # реализации DEV-088, не наблюдалось на SEC-041/SA-073 — их стенды не жили под
    # `.worktrees/`). Относительная цель избавляет от этого без изменения формы паттернов.
