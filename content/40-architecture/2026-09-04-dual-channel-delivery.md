...
Операторская санкция (`.claude-plugin/`…`scripts/`, README, LICENSE) плюс `.github/`,
добавленный Д5 этим решением, плюс `compat.json` — без него команда №2 паритета Д6 (пин
`compat.json`) структурно не может пройти и документированный путь восстановления README
(«Если что-то не работает» → `ktalk-onboard.sh check`) не читает файл, которого нет на
GitHub-канале; `compat.json` несёт только имя и версию пакета (`package_name`/
`package_version`), внутренних данных не содержит. Файл манифеста не публикуется (вне
списка) — читается только внутри исходного дерева самим джобом и самим гейтом.
